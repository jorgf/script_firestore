...
class Firestore {
  ...
  private function writeData($arrValues, $nameDir){
    foreach ($arrValues as $nameDoc => $value) {
      $str=trimString($nameDoc,"projects/{$this->getProjectId()}/databases/(default)/documents/",'');
  ...
  private function setDocuments($arrSub){
    foreach ($arrSub as $key) {
      ...
      $strPath = trimString($key,'bkp_firestore/','/data.json');
  ...
  public function __construct(){
    $this->firestore = new FirestoreClient([
      'keyFilePath' => 'settings.json'
    ]);
  }
  public function getProjectId(){
    $obj = json_decode(FILE::readFile('settings.json'));
